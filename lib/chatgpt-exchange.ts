...
export function parseChatgptLesson(raw: string, lesson: Lesson): ChatgptLesson {
  ...
  let input: unknown;
  try {input=JSON.parse(raw.replace(/^\uFEFF/,"").trim().replace(/^```(?:json)?\s*\n([\s\S]*?)\n```$/i,"$1"));} catch {throw Error("This is not valid JSON. Ask ChatGPT for the completed JSON file, without surrounding commentary.");}
  ...
  if(JSON.stringify(v).includes("REPLACE:"))throw Error("The template still has placeholders. Ask ChatGPT to complete it first.");
